...
<div class="login-wrap">
    <div class="login-card">
        <h2 class="mt-0">{$t('login.title')}</h2>
        <p class="text-sm opacity-70 mt-1 mb-5">{$t('login.subtitle')}</p>
        ...
        <div class="divider text-xs opacity-60">{$t('login.or_email')}</div>

        <form on:submit|preventDefault={sendMagicLink} class="flex flex-col gap-2.5">
            <input class="input input-bordered w-full" type="email" bind:value={email} placeholder={$t('login.email_placeholder')} required />
            <button class="btn btn-primary" type="submit">{$t('login.send_link')}</button>
        </form>

        {#if emailStatus === 'sending'}<p class="mt-3 mb-0 text-sm">{$t('login.sending')}</p>
        {:else if emailStatus === 'sent'}<p class="mt-3 mb-0 text-sm">{$t('login.sent')}</p>
        {:else if emailStatus === 'failed'}<p class="mt-3 mb-0 text-sm">{$t('login.failed')}</p>
        {:else if emailStatus}<p class="mt-3 mb-0 text-sm">{emailStatus}</p>{/if}
        {#if devLink}
            <p class="mt-1 mb-0 text-xs opacity-60">{$t('login.dev_link')} <a class="link" href={devLink}>{$t('login.dev_link_open')}</a></p>
        {/if}
    </div>
</div>
