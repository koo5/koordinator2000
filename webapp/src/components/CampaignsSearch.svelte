<script lang="ts">
			...
			campaigns(
				...
				where: {
					_and: [
						...
						{
							_or: [
								{title: {_ilike: $_search_term}},
								{description: {_ilike: $_search_term}}
							]
						}
						# To enable tag filtering, uncomment and fix:
						# {
						#   campaign_tags: {
						#     tag_id: {_in: $_tag_ids}
						#   }
	...
	$: vars = {
		...
		_limit: itemsPerPage,
		// Tag filter variable - currently disabled in the query
		_tag_ids: selectedTagIds.length > 0 ? selectedTagIds : []
...
<div class="content_block">
		...
		<FormGroup>
			...
			<div class="tags-container mb-3">
				...
				{:else}
					...
					<div class="note mt-2">
						<small class="text-muted">Note: Tag filtering is currently in development</small>
					</div>
				{/if}
			</div>
		</FormGroup>
			...
			<div class="mb-3">
				<div class="form-check form-switch mb-2">
					<input 
						...
						on:change={toggleLocationFilter}
						disabled={true}
					/>
					<label class="form-check-label" for="location-filter">
						Enable location filter (coming soon)
					</label>
				</div>
	...
	<div class="debug-info mt-3">
		<h5>Debug Info</h5>
		<div>
			<strong>Filters:</strong> {JSON.stringify($searchFiltersStore, null, 2)}
		</div>
		<div>
			<strong>Seen:</strong> {JSON.stringify(seen, null, 2)}
		</div>
		<div>
			<strong>Seeing:</strong> {JSON.stringify(seeing, null, 2)}
		</div>
		<div>
			<strong>Available Tags:</strong> {JSON.stringify(availableTags, null, 2)}
		</div>
	</div>
	{/if}
</div>
...
<div class="text-center mt-3 mb-5">
	<button class="btn btn-primary" bind:this={more_button} color="primary" aria-label="more..." on:click={more}>
		Load More
	</button>
</div>
