<script lang="ts">
	...
	const ADD_TAG_TO_CAMPAIGN = gql`
		mutation AddTagToCampaign($campaignId: Int!, $tagId: Int!) {
			insert_campaign_tags_one(object: {
	...
	async function addTagToCampaign() {
		...
		try {
			const result = await client.mutation(ADD_TAG_TO_CAMPAIGN, {
				campaignId,
				tagId: selectedTagId
			}).toPromise();
	...
	async function removeTagFromCampaign(tagId: number) {
		...
		try {
			const result = await client.mutation(REMOVE_TAG_FROM_CAMPAIGN, {
				campaignId,
				tagId
			}).toPromise();
	...
	async function createTag() {
		...
		try {
			const result = await client.mutation(CREATE_TAG, {
				...
				<div class={badgeClass}>
					...
					{#if !readOnly}
						<button 
							...
							on:click={() => removeTagFromCampaign(tag.id)}
