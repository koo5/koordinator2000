<script>
    ...
    $: classes = ['progress-bar', striped ? 'striped' : '', animated && striped ? 'animated' : ''].filter(Boolean).join(' ');
</script>

<div class="progress" style="height: {height}px; background-color: {backgroundColor};">
    <div class={classes} role="progressbar" style="width: {percentage}%; background-color: {color};" aria-valuenow={value} aria-valuemin={min} aria-valuemax={max}>
        {#if showLabel}
