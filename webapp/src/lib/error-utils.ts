 ...
 */
import { error, type HttpError, redirect } from '@sveltejs/kit';
 ...
 */
export function createError(status: number, message: string, details: Record<string, any> = {}): HttpError {
 ...
 */
export function requireAuth(message: string = 'Authentication required', returnUrl: string = '/'): never {
...
export async function handleApiResponse<T = any>(response: Response): Promise<T> {
    if (!response.ok) {
        ...
        throw createError(response.status, errorData.message || 'API request failed', { details: errorData });
    }
