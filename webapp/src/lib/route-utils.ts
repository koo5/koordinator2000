...
import { createError, requireAuth } from './error-utils';
 ...
 */
export async function loadFromApi<T = any>(event: RequestEvent, endpoint: string, options: RequestInit = {}): Promise<T> {
    try {
 ...
 */
export async function navigate(path: string, options: Record<string, any> = {}): Promise<void> {
    try {
