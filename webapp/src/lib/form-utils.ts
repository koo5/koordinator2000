 ...
 */
export function createForm<T extends Record<string, any>>(initialData: T, onSubmit: (data: T) => Promise<any>): FormAPI<T> {
 ...
 */
export function validateForm<T extends Record<string, any>>(data: T, validators: FormValidators<T>): Partial<Record<keyof T, string>> {
...
export const validators: Record<string, ValidatorCreator> = {
    required:
        ...
        (value: any): string | null =>
            !value && value !== 0 && value !== false ? message : null,

    minLength:
        (min: number, message: string = `Must be at least ${min} characters`): Validator =>
        (value: any): string | null =>
            value && value.length < min ? message : null,

    maxLength:
        (max: number, message: string = `Must be at most ${max} characters`): Validator =>
        (value: any): string | null =>
            value && value.length > max ? message : null,

    pattern:
        (regex: RegExp, message: string = 'Invalid format'): Validator =>
        (value: any): string | null =>
            value && !regex.test(value) ? message : null,

    email:
        (message: string = 'Invalid email address'): Validator =>
        (value: any): string | null =>
            value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) ? message : null,

    numeric:
        (message: string = 'Must be a number'): Validator =>
        (value: any): string | null =>
            value && isNaN(Number(value)) ? message : null,

    range:
        (min: number, max: number, message: string = `Must be between ${min} and ${max}`): Validator =>
        (value: any): string | null => {
