 ...
 */
export async function serverQuery<TData = any, TVariables extends Record<string, any> = Record<string, any>>(queryDocument: DocumentNode, variables: TVariables = {} as TVariables): Promise<OperationResult<TData>> {
    try {
        const client = getServerClient();
        const result = await client.query<TData, TVariables>(queryDocument, variables).toPromise();
        return result;
    } catch (error) {
 ...
 */
export async function serverMutation<TData = any, TVariables extends Record<string, any> = Record<string, any>>(mutationDocument: DocumentNode, variables: TVariables = {} as TVariables): Promise<OperationResult<TData>> {
    try {
        const client = getServerClient();
        const result = await client.mutation<TData, TVariables>(mutationDocument, variables).toPromise();
        return result;
    } catch (error) {
